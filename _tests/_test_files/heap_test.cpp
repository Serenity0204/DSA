#include "gtest/gtest.h"
#include <iomanip>
#include <iostream>

//------------------------------------------------------------------------------
// Files we are testing:
#include "../../includes/Heap/heap.h"

//------------------------------------------------------------------------------

using namespace std;

//------------------------------------------------------------------------------
//          COPY BASIC_TEST INTO THIS FILE.
//                    AND THEN,
//         DO NOT EDIT THIS FILE ANY FURTHER
//------------------------------------------------------------------------------

int random(int low, int high)
{
    // call srand in main if you want better random numbers.
    return static_cast<int>(low + (rand() % static_cast<int>((high - low + 1))));
}

template <class Item>
bool is_sorted(Item* arr, int size)
{
    for (int i = 0; i < size - 1; ++i)
        if (arr[i] > arr[i + 1]) return false;
    return true;
}

const string LINE = "\n" + string(50, '-') + "\n";
const string DASH = "\n" + string(50, '.') + "\n";

bool test_heap(bool debug = false)
{
    // initial heap
    DSA::Heap<int> heap;

    cout << "Heap Test" << LINE;
    cout << "heap is empty: " << boolalpha << heap.is_empty() << endl
         << endl;

    // insert to heap
    int size = 21;
    for (int i = 0; i < size; i++)
    {
        heap.insert(random(10, 100));
        if (!heap.verify())
        {
            cout << "error line 44" << endl;
            return false;
        }
    }

    cout << heap << LINE << endl;
    cout << "heap is empty: " << boolalpha << heap.is_empty() << endl;
    cout << "number of elements on the heap: " << heap.size() << endl;
    cout << "the tree is " << (heap.verify() ? "" : "in") << "valid" << endl;
    if (!heap.verify())
    {
        cout << "error line 56" << endl;
        return false;
    }
    int* heap_array = heap.heap_array();
    int* walker = heap_array;

    cout << "heap array: " << endl;
    for (int i = 0; i < heap.size(); i++)
        cout << "|" << *walker++;
    cout << "|" << endl;
    cout << endl;

    // // remove one random node from heap if node is in the heap, otherwise do nothing
    for (int i = 0; i < size * 2; i++)
    {
        heap.remove_node(random(10, 100));
        if (!heap.verify())
        {
            cout << "error line 70" << endl;
            return false;
        }
    }
    cout << "number of elements on the tree: " << heap.size() << endl;
    cout << "the tree is " << (heap.verify() ? "" : "in") << "valid" << endl;
    cout << "the tree: " << endl
         << endl
         << heap << DASH << endl;

    // remove top from heap
    cout << LINE << endl
         << "Test pop()" << endl
         << LINE << endl;
    while (!heap.is_empty())
    {
        cout << "removed: " << heap.top() << endl;
        heap.remove();
        if (heap.is_empty())
            cout << "tree is now empty." << endl;
        else
            cout << "top of the heap: " << heap.top() << endl;
        cout << "number of elements on the heap: " << heap.size() << endl;
        cout << "the tree is " << (heap.verify() ? "" : "in") << "valid" << endl;
        cout << "the new tree: " << endl
             << endl
             << heap << DASH << endl;
        if (!heap.verify())
        {
            cout << "error line 93" << endl;
            return false;
        }
    }

    // initial array
    int arr[50];
    for (int i = 0; i < size; i++)
        arr[i] = random(10, 100);

    // initial heap by array
    DSA::Heap<int> heap2(arr, size);
    cout << "size:" << size << endl;
    cout << "number of elements on the tree 2: " << heap2.size() << endl;
    cout << "the tree 2 is " << (heap2.verify() ? "" : "in") << "valid" << endl;
    cout << "the tree 2: " << endl
         << endl
         << heap2 << DASH << endl;
    if (!heap2.verify())
    {
        cout << "error line 110" << endl;
        return false;
    }
    // initial heap by another heap
    DSA::Heap<int> heap3(heap2);
    cout << "number of elements on the tree 3: " << heap3.size() << endl;
    cout << "the tree 3 is " << (heap3.verify() ? "" : "in") << "valid" << endl;
    cout << "the tree 3: " << endl
         << endl
         << heap3 << DASH << endl;
    if (!heap3.verify())
    {
        cout << "error line 121" << endl;
        return false;
    }
    // create heap by another heap
    DSA::Heap<int> heap4;
    heap4 = heap3;
    cout << "number of elements on the tree 4: " << heap4.size() << endl;
    cout << "the tree 4 is " << (heap4.verify() ? "" : "in") << "valid" << endl;
    cout << "the tree 4: " << endl
         << endl
         << heap4 << DASH << endl;
    if (!heap4.verify())
    {
        cout << "error line 132" << endl;
        return false;
    }
    // heap sort
    heap_array = heap4.heap_array();
    walker = heap_array;
    cout << "heap 4 array: " << endl;
    for (int i = 0; i < heap4.size(); i++)
        cout << "|" << *walker++;
    cout << "|" << endl;

    int* sorted_array = heap4.heap_sort();
    walker = sorted_array;
    cout << "heap 4 sorted array: " << endl;
    for (int i = 0; i < heap4.size(); i++)
        cout << "|" << *walker++;
    cout << "|" << endl;
    cout << endl;
    if (!is_sorted(sorted_array, heap4.size()))
    {
        cout << "not sorted in line 152" << endl;
        return false;
    }
    delete[] heap_array;
    delete[] sorted_array;

    return true;
}

// Lord help me!
const bool debug = false;

TEST(BASIC_TES, TestHeap)
{
    bool success = test_heap(debug);
    EXPECT_EQ(success, true);
}

int main(int argc, char** argv)
{
    ::testing::InitGoogleTest(&argc, argv);
    std::cout << "\n\n----------running basic_test.cpp---------\n\n"
              << std::endl;
    return RUN_ALL_TESTS();
}

/*
includes
└── heap
    └── heap.h

1 directory, 1 file

----------running basic_test.cpp---------

Heap Test
--------------------------------------------------
heap is empty: true

            31
        73
            65
    82
            54
        73
            33
86
            65
        72
                28
            41
                32
    72
                21
            66
                42
        67
                39
            40
                36

--------------------------------------------------

heap is empty: false
number of elements on the heap: 21
the tree is valid
heap array:
|86|72|82|67|72|73|73|40|66|41|65|33|54|65|31|36|39|42|21|32|28|

number of elements on the tree: 12
the tree is valid
the tree:

        65
    73
        33
            21
82
            28
        65
            41
    72
            42
        67
            32

..................................................


--------------------------------------------------

Test pop()

--------------------------------------------------

removed: 82
top of the heap: 73
number of elements on the heap: 11
the tree is valid
the new tree:

        21
    65
        33
73
            28
        65
            41
    72
            42
        67
            32

..................................................

removed: 73
top of the heap: 72
number of elements on the heap: 10
the tree is valid
the new tree:

        21
    65
        33
72
        65
            41
    67
            28
        42
            32

..................................................

removed: 72
top of the heap: 67
number of elements on the heap: 9
the tree is valid
the new tree:

        21
    65
        33
67
        41
    65
            28
        42
            32

..................................................

removed: 67
top of the heap: 65
number of elements on the heap: 8
the tree is valid
the new tree:

        21
    65
        33
65
        41
    42
        32
            28

..................................................

removed: 65
top of the heap: 65
number of elements on the heap: 7
the tree is valid
the new tree:

        21
    33
        28
65
        41
    42
        32

..................................................

removed: 65
top of the heap: 42
number of elements on the heap: 6
the tree is valid
the new tree:

    33
        28
42
        21
    41
        32

..................................................

removed: 42
top of the heap: 41
number of elements on the heap: 5
the tree is valid
the new tree:

    33
41
        21
    32
        28

..................................................

removed: 41
top of the heap: 33
number of elements on the heap: 4
the tree is valid
the new tree:

    21
33
    32
        28

..................................................

removed: 33
top of the heap: 32
number of elements on the heap: 3
the tree is valid
the new tree:

    21
32
    28

..................................................

removed: 32
top of the heap: 28
number of elements on the heap: 2
the tree is valid
the new tree:

28
    21

..................................................

removed: 28
top of the heap: 21
number of elements on the heap: 1
the tree is valid
the new tree:

21

..................................................

removed: 21
tree is now empty.
number of elements on the heap: 0
the tree is valid
the new tree:


..................................................

number of elements on the tree 2: 21
the tree 2 is valid
the tree 2:

            25
        80
            55
    90
            54
        67
            24
99
            30
        87
                29
            52
                38
    93
                39
            62
                58
        86
                17
            57
                31

..................................................

number of elements on the tree 3: 21
the tree 3 is valid
the tree 3:

            25
        80
            55
    90
            54
        67
            24
99
            30
        87
                29
            52
                38
    93
                39
            62
                58
        86
                17
            57
                31

..................................................

number of elements on the tree 4: 21
the tree 4 is valid
the tree 4:

            25
        80
            55
    90
            54
        67
            24
99
            30
        87
                29
            52
                38
    93
                39
            62
                58
        86
                17
            57
                31

..................................................

heap 4 array:
|99|93|90|86|87|67|80|57|62|52|30|24|54|55|25|31|17|58|39|38|29|
heap 4 sorted array:
|17|24|25|29|30|31|38|39|52|54|55|57|58|62|67|80|86|87|90|93|99|

[       OK ] BASIC_TES.TestHeap (7 ms)
[----------] 1 test from BASIC_TES (7 ms total)

[----------] Global test environment tear-down
[==========] 1 test from 1 test suite ran. (25 ms total)
[  PASSED  ] 1 test.
*/