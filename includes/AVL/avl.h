...
namespace DSA
{
    ...
    public:
        ...
        AVL(AVLUtils::binary_tree_node<Item>* root)
        {
            this->_root_ptr = AVLUtils::tree_copy<Item>(root);
        ...
        AVLUtils::binary_tree_node<Item>*& get_root() { return this->_root_ptr; }
        AVLUtils::binary_tree_node<Item>* root() { return this->_root_ptr; }

        AVLUtils::binary_tree_node<Item>* search(const Item& target) const;
        void insert(const Item& val);
        void erase(const Item& val);
        void clear_all();
        size_t size() { return this->_size; }
        // CONST FUNCTIONS
        const AVLUtils::binary_tree_node<Item>* root() const { return this->_root_ptr; }
        bool empty() const;
    ...
    private:
        int _size;
        AVLUtils::binary_tree_node<Item>* _root_ptr; // Root pointer of binary search tree
    };
    ...
    {
        this->_root_ptr = AVLUtils::sortedArrayToAVL<Item>(nums, 0, size - 1);
        if (this->_root_ptr == nullptr) this->_size = 0;
        if (this->_root_ptr != nullptr)
        {
            this->_root_ptr->update_entire_tree_height();
            this->_size = AVLUtils::tree_size<Item>(this->_root_ptr);
        }
    ...
    {
        this->_root_ptr = AVLUtils::tree_copy<Item>(copy_me._root_ptr);
        if (this->_root_ptr == nullptr) this->_size = 0;
        if (this->_root_ptr != nullptr)
        {
            this->_root_ptr->update_entire_tree_height();
    ...
    AVL<Item>::~AVL()
    {
        AVLUtils::tree_clear<Item>(this->get_root());
    }
    ...
    AVL<Item>& AVL<Item>::operator=(const AVL<Item>& rhs)
    {
        ...
        AVLUtils::tree_clear<Item>(this->_root_ptr);
        this->_root_ptr = AVLUtils::tree_copy<Item>(rhs._root_ptr);
        if (this->_root_ptr == nullptr) this->_size = 0;
        if (this->_root_ptr != nullptr)
        {
            this->_root_ptr->update_entire_tree_height();
    ...
    // MODIFICATION FUNCTIONS
    template <class Item>
    AVLUtils::binary_tree_node<Item>* AVL<Item>::search(const Item& target) const
    {
